...
public class MotionProfileLoggingService {
	...
	public static void process(List<MotionProfileStatus> statusList) {
		double now = edu.wpi.first.wpilibj.Timer.getFPGATimestamp();
		MotionProfileStatus mps = statusList.get(0);
		if (mps.activePoint.isLastPoint) {
			System.out.println("Last Point achieved");
		}
		if((now-timeout) > 0.2){
			timeout = now;
			...
			System.out.format("%-9s\t", mps.topBufferCnt);
			System.out.format("%-9s\t", mps.btmBufferCnt);
			System.out.format("%-9s\t", StrOutputEnable(mps.outputEnable));
			System.out.format("%-9s\t", (mps.hasUnderrun ? "1" : ""));
			System.out.format("%-9s\t", (mps.isUnderrun ? "1" : ""));
			System.out.format("%-9s\t", (mps.activePointValid ? "1" : ""));
			System.out.format("%-9s\t", (mps.activePoint.isLastPoint ? "1" : ""));
			System.out.format("%-9s\t", (mps.activePoint.velocityOnly ? "1" : ""));
			System.out.format("%-9s\t", round(mps.activePoint.position));
			System.out.format("%-9s\t", round(mps.activePoint.velocity));
